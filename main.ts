...
function rndNum(min: number, max: number) {
...
function rndProb(prob: number): boolean {
    if (rndInt(0,100) < prob) return true;
...
function hexadecimal(num: number, length: number): string {
...
function shortenNumber(num: number): string {
    ...
    if (num > 1_000_000_000_000_000) {
        abbreviatedNumber = Math.round(num / 1_000_000_000_000_0)
        abbreviatedNumber = abbreviatedNumber / 100
        ending = 'QD'
    } else if (num > 1_000_000_000_000) {
        abbreviatedNumber = Math.round(num / 10_000_000_000)
        abbreviatedNumber = abbreviatedNumber / 100
        ending = 'T'
...
function colourValue(colour: RGB): number {
    let value = 0
    let r = colour[0]
    let g = colour[1]
    let b = colour[2]
...
class Disease {
    constructor (public lethality: number, public infectivity: number, public severity: number, public countdown: number) {
    }

    public mutate(): Disease {
...
function generateDisease(progress: number): Disease {
...
class Nature {
    constructor (public tolerance: number, public sociality: number, public enhance: number) {}
    mutate() {
...
class Animal {
    income: number
    constructor (
    public name: string, public colour: RGB, public rarity: number,
    public level: number, public nature: Nature, public diet: Diet,
    public sickness: Disease | null = null, public value: number,
    public hunger: number, public ancestory: string[]) {
        if (rarity > 16777215) {
            rarity = 16777215
        }
        let reduction = 0
    ...
    public infect(disease: Disease): boolean {
    ...
    public breed(partner: Animal): Animal {
...
class Egg {
    constructor (public duration: number = 10) {}

    public tick(): Animal | 0 {
        this.duration-=1
        if (this.duration < 1) {
            return this.hatch()
        }
...
function decodeGenome(rawGenome: string): Organism {
    ...
    let ancestory = []
    let ancestoryGenome: string = genome.slice(62)
...
type Organism = Egg | Animal

/*--------------- GAME ---------------*/

interface EnhancementItems {
    meat: number,
    grass: number
}
...
interface UpgradeList {
    ...
    tier: number
    eggSpeed: number
    toleranceMutation: number
    colourEggs: number
    colourMutation: number
}

class Game {
    gameView: GameView;
    constructor (public progress: number, public coins: number, public animals: Organism[], public enhancementItems: EnhancementItems, private upgrds: any) {
    ...
    public addAnimal (organism: Organism) {
    ...
    public removeAnimal (organism: Organism) {
    ...
    public getSocial (baseAnimal: string): number {
        let total = 0
    ...
    public get unlockedUpgrades(): UpgradeList {
    ...
    public get upgrades(): UpgradeList {
        let data: any = {}
    ...
    public purchaseUpgrade(upgrade: string) {
...
class GameView {
    cardHolder: HTMLElement | null;
    cards: Map<Organism, any>
    coins: HTMLElement;
    meat: HTMLElement;
    grass: HTMLElement;
    eggCost: HTMLElement;
    breedCost: HTMLElement;
    constructor (coins: number, meat: number, grass: number, progress: number, animals: Organism[]) {
        this.cardHolder = document.getElementById('card-holder')

        //@ts-ignore: These will never be null
        this.coins = document.getElementById('coins'); this.meat = document.getElementById('meat'); this.grass = document.getElementById('grass'); this.breedCost = document.getElementById('breed-cost-info'); this.eggCost = document.getElementById('egg-cost-info');
    ...
    public setCoins (e: number) {
        this.coins.innerText = shortenNumber(e)
    }
    public setMeat (e: number) {
        this.meat.innerText = e.toString()
    }
    public setGrass (e: number) {
        this.grass.innerText = e.toString()
    }

    public addCard(contents: Organism) {
    ...
    public removeCard(contents: Organism) {
        this.cards.get(contents).elem.remove()
        this.cards.delete(contents)
    }

    public updateCard(contents: Organism) {
        if (contents instanceof Egg) {
...
function closeAction() {
    ...
    let breed: HTMLDivElement = document.getElementById("breed"); let sell: HTMLDivElement = document.getElementById("sell"); let feed: HTMLDivElement = document.getElementById('feed');
...
function refreshShop() {
    ...
    for (let upgrade of Object.keys(game.upgrades)) {
        ...
        shopItemName.innerText = game.upgrades[upgrade].name; shopItemDescr.innerText = game.upgrades[upgrade].descr; shopItemCost.innerText = shortenNumber(game.upgrades[upgrade].cost); 
...
function toggleShop() {
    refreshShop()
    let shop = document.getElementById('shop')
    shop?.classList.toggle('enabled')
}

function togglePause() {
    //@ts-ignore: Trust me bro
    let pauseImg: HTMLImageElement = document.getElementById('pause-img')
...
function toggleSearch() {
    ...
    let search: HTMLDivElement = document.getElementById('search'); let act: HTMLDivElement = document.getElementById('actions'); 
...
function notification(header: string, details: string) {
    ...
    notificationHeader.innerText = header; notificationDetails?.innerText = details;
...
function handleCardClick(organism: Animal, elem: any) {
// BREEDING
    if (currentAction == "Breed") {
        if (game.animals.length >= game.unlockedUpgrades.animalCount) {
            notification('NO SPACE', 'You have run out of space for animals. You can get more space by purchasing it from the shop or by selling animals.')
...
function GameEventHandler() {
    //@ts-ignore: This exists
    let eggPurchase: HTMLDivElement = document.getElementById('purchase-egg')
    eggPurchase.addEventListener('click', purchaseEgg)

    //@ts-ignore: These exists
    let breed: HTMLDivElement = document.getElementById("breed"); let sell: HTMLDivElement = document.getElementById("sell"); let feed: HTMLDivElement = document.getElementById('feed'); let shop: HTMLDivElement = document.getElementById('shop-button'); let pause: HTMLDivElement = document.getElementById('pause'); let searchButton: HTMLDivElement = document.getElementById('search-button'); let search: HTMLInputElement = document.getElementById('search-input'); let restart: HTMLDivElement = document.getElementById('restart'); let bin: HTMLDivElement = document.getElementById('bin')
...
let game: Game;
let currentAction: "Breed" | "Sell" | "Feed" | null = null
let breedingPartner: Animal | null = null
let favourites: any[] = []
...
function sendAnimal(genome: string) {
    socket.send(`animal ${genome}`)
}
function removeAnimal(genome: string) {
